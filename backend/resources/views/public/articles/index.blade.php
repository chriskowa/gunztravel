...
@section('meta_description', 'Artikel terbaru dari Gunz Travel.')

@section('content')
    <div class="rounded-3xl border border-gray-200 bg-gradient-to-br from-brand-50 via-white to-white p-6 shadow-sm sm:p-10">
        <h1 class="text-3xl font-black tracking-tight text-gray-900 sm:text-4xl">Artikel</h1>
        <p class="mt-3 max-w-2xl text-gray-600">Update seputar travel, tips perjalanan, dan info layanan.</p>
    </div>
                    ...
                    @if ($post->tags->count())
                        <div class="mt-4 flex flex-wrap gap-2">
                            @foreach ($post->tags->take(3) as $tag)
                                <span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-bold text-gray-700">#{{ $tag->name }}</span>
                            @endforeach
                        </div>
                    @endif
